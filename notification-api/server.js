...
app.post('/subscribe', async (req, res) => {
  try {
    ...
    const existingSubscriptions = await directusFetch(
      `/items/push_subscriptions?filter[endpoint][_eq]=${encodeURIComponent(subscription.endpoint)}`
    );

    if (existingSubscriptions && existingSubscriptions.length > 0) {
      // Update existing subscription
      await directusFetch(`/items/push_subscriptions/${existingSubscriptions[0].id}`, {
        method: 'PATCH',
      ...
      await directusFetch('/items/push_subscriptions', {
        method: 'POST',
        body: subscriptionData
      });
...
app.post('/unsubscribe', async (req, res) => {
  try {
    ...
    if (subscriptions && subscriptions.length > 0) {
      await directusFetch(`/items/push_subscriptions/${subscriptions[0].id}`, {
        method: 'DELETE'
      });
...
app.post('/send', async (req, res) => {
  try {
    ...
    if (userIds && userIds.length > 0) {
      ...
      subscriptions = await directusFetch(`/items/push_subscriptions?${userFilter}`);
    } else {
      // Send to all subscribed users
      subscriptions = await directusFetch('/items/push_subscriptions?limit=-1');
    }
    ...
    const results = await Promise.allSettled(
      subscriptions.map(async (sub) => {
        try {
          ...
          await directusFetch(`/items/push_subscriptions/${sub.id}`, {
            method: 'PATCH',
...
app.post('/notify-new-event', async (req, res) => {
  try {
    ...
    const subscriptions = await directusFetch('/items/push_subscriptions?limit=-1');
...
app.post('/notify-event-reminder', async (req, res) => {
  try {
    ...
    const userFilter = userIds.map(id => `filter[user_id][_eq]=${id}`).join('&');
    const subscriptions = await directusFetch(`/items/push_subscriptions?${userFilter}`);
