...
export default function CalendarView({
    ...
    return (
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
            {/* Header */}
            <div className="p-6 flex items-center justify-between bg-paars text-white">
                    ...
                    return (
                        <div
                            ...
                            className={`min-h-[120px] p-2 flex flex-col gap-1 transition-colors cursor-pointer
                                ...
                                ${isSelected ? 'ring-2 ring-inset ring-oranje bg-oranje/5' : ''}
                            `}
                        >
                            <div className="flex justify-between items-start">
                                <span
                                    className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full
                                        ${isDayToday ? 'bg-paars text-white' : isSelected ? 'text-oranje' : 'text-gray-700'}
                                    `}
                                >
                                    {format(day, 'd')}
                                </span>
                            ...
                            <div className="flex flex-col gap-1 mt-1 overflow-y-auto max-h-[100px] custom-scrollbar">
                                {dayEvents.map(event => (
                                    <button
                                        ...
                                        className="text-left text-xs p-1.5 rounded bg-roze/10 hover:bg-roze/20 text-paars font-medium truncate transition-all hover:scale-[1.02]"
