...
export const intro_group_attendance_log = pgTable("intro_group_attendance_log", {
	...
	index("idx_intro_group_attendance_log_member_date").using("btree", table.intro_group_member_id.asc().nullsLast().op("int4_ops"), table.date.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.intro_group_member_id],
			foreignColumns: [intro_group_members.id],
...
export const nda_signatures = pgTable("nda_signatures", {
	...
	index("idx_nda_signatures_user_status").using("btree", table.user_id.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.nda_template_id],
			foreignColumns: [nda_templates.id],
			name: "nda_signatures_nda_template_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.committee_id],
			foreignColumns: [committees.id],
			name: "nda_signatures_committee_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.user_id],
			foreignColumns: [directus_users.id],
			name: "nda_signatures_user_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.member_signature],
			foreignColumns: [directus_files.id],
			name: "nda_signatures_member_signature_fkey"
		}).onDelete("set null"),
	foreignKey({
			columns: [table.signed_document],
			foreignColumns: [directus_files.id],
			name: "nda_signatures_signed_document_fkey"
		}).onDelete("set null"),
]);
...
export const nda_settings = pgTable("nda_settings", {
	...
	foreignKey({
			...
			foreignColumns: [directus_users.id],
			name: "nda_settings_secretary_user_id_fkey"
		}).onDelete("set null"),
]);
...
export const nda_templates = pgTable("nda_templates", {
	...
	foreignKey({
			columns: [table.committee_id],
			foreignColumns: [committees.id],
			name: "nda_templates_committee_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.document],
			foreignColumns: [directus_files.id],
			name: "nda_templates_document_fkey"
		}).onDelete("set null"),
	foreignKey({
			columns: [table.secretary_user_id],
			foreignColumns: [directus_users.id],
			name: "nda_templates_secretary_user_id_fkey"
		}).onDelete("set null"),
	foreignKey({
			columns: [table.user_created],
			foreignColumns: [directus_users.id],
			name: "nda_templates_user_created_fkey"
		}).onDelete("set null"),
...
export const intro_settings = pgTable("intro_settings", {
	...
	student_signups_open: boolean().default(false),
	parent_signups_open: boolean(),
}, (table) => [
	foreignKey({
