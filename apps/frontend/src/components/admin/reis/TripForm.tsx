...
// Interface aangepast om FormDataEntryValue (inclusief File) te accepteren
interface ActionState {
    success: boolean;
    id?: number;
    error?: string;
    fieldErrors?: Record<string, string[]>;
    initialData?: Record<string, string | number | boolean | null | undefined | unknown>;
}
...
export default function TripForm({
    ...
    }, [state?.success, editingTrip, onSuccess]);
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="bg-[var(--beheer-card-bg)] rounded-[var(--beheer-radius)] shadow-xl border border-[var(--beheer-border)] overflow-hidden">
                            ...
                            <div className="p-6 space-y-6">
                                <div>
                                    <label htmlFor="name" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Naam van de reis *</label>
                                    <input
                                        type="text"
                                        id="name"
                                        name="name"
                                ...
                                <div>
                                    <label htmlFor="description" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Beschrijving</label>
                                    <textarea
                                        id="description"
                                        name="description"
                                        rows={4}
                        ...
                        {/* Section: Planning & capaciteit */}
                        <div className="bg-[var(--beheer-card-bg)] rounded-[var(--beheer-radius)] shadow-xl border border-[var(--beheer-border)] overflow-hidden">
                            <div className="px-6 py-4 border-b border-[var(--beheer-border)] bg-[var(--beheer-card-soft)]/50 flex items-center gap-3">
                                <CalendarIcon className="h-4 w-4 text-[var(--beheer-accent)]" />
                                <h2 className="text-base font-semibold text-[var(--beheer-text)]">Planning & capaciteit</h2>
                            </div>
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="start_date" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Startdatum *</label>
                                        <input
                                            type="date"
                                            id="start_date"
                                            name="start_date"
                                            className={`beheer-input ${formErrors.start_date ? 'border-red-500' : ''}`}
                                            defaultValue={state?.initialData?.start_date as string || editingTrip?.start_date?.split('T')[0] || ''}
                                        />
                                        {formErrors.start_date && <p className="text-red-500 text-sm font-semibold mt-2">{formErrors.start_date[0]}</p>}
                                    </div>
                                    <div>
                                        <label htmlFor="end_date" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Einddatum</label>
                                        <input
                                            type="date"
                                            id="end_date"
                                            name="end_date"
                                            className="beheer-input"
                                            defaultValue={state?.initialData?.end_date as string || editingTrip?.end_date?.split('T')[0] || ''}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-[var(--beheer-border)]/30">
                                    <div>
                                        <label htmlFor="max_participants" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Max. deelnemers *</label>
                                        <input
                                            type="number"
                                            id="max_participants"
                                            name="max_participants"
                                            min="0"
                                            className={`beheer-input ${formErrors.max_participants ? 'border-red-500' : ''}`}
                                            placeholder="Bijv. 50"
                                            defaultValue={state?.initialData?.max_participants as string || editingTrip?.max_participants || ''}
                                        />
                                        {formErrors.max_participants && <p className="text-red-500 text-sm font-semibold mt-2">{formErrors.max_participants[0]}</p>}
                                    </div>
                                    <div>
                                        <label htmlFor="registration_start_date" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Auto-open datum</label>
                                        <input
                                            type="datetime-local"
                        ...
                        <div className="bg-[var(--beheer-card-bg)] rounded-[var(--beheer-radius)] shadow-xl border border-[var(--beheer-border)] overflow-hidden">
                            <div className="px-6 py-4 border-b border-[var(--beheer-border)] bg-[var(--beheer-card-soft)]/50 flex items-center gap-3">
                                ...
                                <h2 className="text-base font-semibold text-[var(--beheer-text)]">Financiën</h2>
                            </div>
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="base_price" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Totale prijs (€) *</label>
                                        <input
                                            type="number"
                                            id="base_price"
                                            name="base_price"
                                            step="0.01"
                                            min="0"
                                            className={`beheer-input ${formErrors.base_price ? 'border-red-500' : ''}`}
                                            placeholder="0.00"
                                            defaultValue={state?.initialData?.base_price as string || editingTrip?.base_price || ''}
                                        />
                                        {formErrors.base_price && <p className="text-red-500 text-sm font-semibold mt-2">{formErrors.base_price[0]}</p>}
                                    </div>
                                    <div>
                                        <label htmlFor="deposit_amount" className="block text-base font-semibold text-[var(--beheer-text-muted)] mb-2">Aanbetaling (€) *</label>
                                        <input
                                            type="number"
                                            id="deposit_amount"
                                            name="deposit_amount"
                                            step="0.01"
                                            min="0"
                                            className={`beheer-input ${formErrors.deposit_amount ? 'border-red-500' : ''}`}
                                            placeholder="0.00"
                                            defaultValue={state?.initialData?.deposit_amount as string || editingTrip?.deposit_amount || ''}
                                        />
                                        {formErrors.deposit_amount && <p className="text-red-500 text-sm font-semibold mt-2">{formErrors.deposit_amount[0]}</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
