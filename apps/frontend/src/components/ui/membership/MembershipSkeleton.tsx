...
export default function MembershipSkeleton({ isGuest = true }: { isGuest?: boolean }) {
    return (
        <>
            <PageHeader
            ...
            <main className="max-w-app mx-auto">
                <div className="flex flex-col sm:flex-row gap-6 px-6 py-8 sm:py-10 md:py-12">
                    <section className={`bg-[var(--bg-card)] dark:border dark:border-white/10 rounded-[2rem] shadow-xl p-6 sm:p-10 ${isGuest ? 'w-full sm:w-1/2' : 'w-full max-w-2xl mx-auto'}`} aria-busy="true">
                        <h1 className="text-4xl font-black text-theme-purple dark:text-purple-400 mb-8 tracking-tight opacity-50">
                            {isGuest ? 'INSCHRIJVEN' : 'LIDMAATSCHAP'}
                        </h1>
