    ...
    return (
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${columnClass} gap-4 mb-10 w-full`}>
            {items.map((stat, i) => (
                <div 
                    key={i}
                    className={`bg-[var(--beheer-card-bg)] border border-[var(--beheer-border)] rounded-[var(--beheer-radius)] p-6 shadow-sm flex flex-col justify-between min-h-[120px] transition-all group ${isLoading ? 'skeleton-active' : 'hover:border-[var(--beheer-accent)]/30'}`}
                    aria-busy={isLoading}
                >
