                ...
                <div className="grid grid-cols-3 bg-(--beheer-card-soft) p-1 rounded-xl border border-(--beheer-border)">
                    {(['draft', 'open', 'closed'] as const).map((value) => (
                        <button
                            ...
                            className={`py-3 px-4 rounded-lg text-xs font-semibold transition-all cursor-pointer capitalize ${status === value ? 'bg-(--beheer-accent) text-white shadow-lg' : 'text-(--beheer-text-muted) hover:text-(--beheer-text)'}`}
                        >
