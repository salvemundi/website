        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <div className="flex justify-end gap-3 mb-8">
                <button
                    ...
                    className="flex items-center justify-center gap-2 px-8 py-2 bg-[var(--beheer-card-bg)] border border-[var(--beheer-border)] text-[var(--beheer-text)] rounded-[var(--beheer-radius)] text-[10px] font-semibold hover:border-[var(--beheer-accent)]/50 transition-all active:scale-95 disabled:opacity-50 shadow-sm"
                >
                    ...
                    {statuses.map((service) => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className={`flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] font-semibold ${getStatusColor(service.status)}`}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        <div className="flex items-center gap-1.5 text-[10px] font-semibold text-[var(--beheer-text-muted)]">
                                ...
                                {service.error && (
                                    <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-2xl">
                                        <p className="text-[10px] font-semibold text-red-500">Error Detail</p>
                                        <p className="text-xs font-semibold text-red-400/80 mt-1">{service.error}</p>
                                    </div>
                                )}
                            </div>
            ...
            {lastUpdated && (
                <p className="mt-8 text-center text-[10px] font-semibold text-[var(--beheer-text-muted)]">
                    {`Laatst gecontroleerd: ${formatDate(lastUpdated!, 'dd-MM-yyyy HH:mm')}`}
                </p>
            )}
        </div>
    );
