...
export default function CouponRow({ 
    ...
    return (
        <tr className="hover:bg-[var(--beheer-card-soft)]/50 transition-colors border-b border-[var(--beheer-border)] last:border-0 opacity-100 group">
            {/* Code */}
            <td className="px-8 py-6 whitespace-nowrap">
                <div className="flex items-center gap-4">
                    ...
                    <span className={`font-black font-mono tracking-widest text-[11px] uppercase ${isExpiredOrMaxed ? 'text-[var(--beheer-text-muted)] opacity-60' : 'text-[var(--beheer-text)]'}`}>
                        {coupon.coupon_code}
                    </span>
                </div>
            </td>
            ...
            {/* Discount */}
            <td className="px-8 py-6 whitespace-nowrap">
                <div className={`flex items-center gap-2 font-black uppercase text-[10px] tracking-widest ${isExpiredOrMaxed ? 'text-[var(--beheer-text-muted)] opacity-50' : 'text-[var(--beheer-accent)]'}`}>
            ...
            <td className="px-8 py-6 whitespace-nowrap hidden sm:table-cell">
                <div className={`flex flex-col gap-2.5 ${isExpiredOrMaxed ? 'opacity-40' : ''}`}>
                    <span className="font-black text-[9px] uppercase tracking-[0.15em] text-[var(--beheer-text)]">
            ...
            <td className="px-8 py-6 whitespace-nowrap text-center">
                <span className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.15em] shadow-sm transform transition-transform group-hover:scale-105 ${status.color}`} title={status.description}>
