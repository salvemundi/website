                    ...
                    <thead>
                        <tr className="bg-(--beheer-card-soft)/50 border-b border-(--beheer-border)/50">
                            <th className="p-4 w-12 text-center">
                                <button onClick={onToggleSelectAll} className="text-(--beheer-text-muted) hover:text-(--beheer-accent) transition-colors">
                            ...
                            filteredSignups.map(s => (
                                <tr key={s.id} className={`hover:bg-(--beheer-accent)/2 transition-colors group ${selectedIds.has(s.id) ? 'bg-(--beheer-accent)/5' : ''}`}>
                                    <td className="p-4 text-center">
                                        <button onClick={() => onToggleSelectOne(s.id)} className={`transition-colors ${selectedIds.has(s.id) ? 'text-(--beheer-accent)' : 'text-(--beheer-text-muted)/30'}`}>
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center justify-end gap-3">
                                            <button 
                                                ...
                                                className="p-2 bg-(--beheer-active)/10 text-(--beheer-active) rounded-xl hover:bg-(--beheer-active) hover:text-white transition-all disabled:opacity-50 border border-(--beheer-active)/20"
                                            >
                                            ...
                                            <button 
                                                ...
                                                disabled={!!isProcessing}
                                                className="p-2 bg-(--beheer-inactive)/10 text-(--beheer-inactive) rounded-xl hover:bg-(--beheer-inactive) hover:text-white transition-all disabled:opacity-50 border border-(--beheer-inactive)/20"
                                            >
