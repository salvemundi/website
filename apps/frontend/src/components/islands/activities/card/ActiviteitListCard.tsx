...
export default function ActiviteitListCard({
    ...
    return (
        <div
            ...
            className={`group relative z-0 overflow-hidden w-full rounded-[1.75rem] bg-(--bg-card) dark:border dark:border-white/10 p-0 shadow-sm transition-all cursor-pointer hover:shadow-md hover:-translate-y-1 flex flex-col md:flex-row ${isPast ? 'opacity-75 grayscale-50' : ''}`}
        >
            {/* Image Section */}
            ...
            <div className="p-5 flex-1 flex flex-col">
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8 flex-1">
                    ...
                    <div className="flex flex-row items-center gap-6 lg:gap-10">
                        <div className="text-left lg:text-right min-w-[140px] shrink-0">
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className="text-xs font-medium text-(--text-muted) whitespace-nowrap overflow-hidden text-ellipsis max-w-[180px]">
                                    {timeRange ? `${timeRange} • ` : ''}{location || 'Locatie volgt'}
                                </p>
                            </div>
                        </div>

                        <div className="bg-(--bg-soft) px-4 py-2 rounded-xl border border-(--border-color) text-center min-w-[80px] ml-auto">
                ...
                <div className="flex flex-wrap justify-end gap-2 mt-4 pt-4 border-t border-(--border-color)/10">
                    <button
                        ...
                        className="px-4 py-2 text-sm font-semibold rounded-full text-(--theme-purple) hover:bg-(--theme-purple) hover:text-white transition"
                    >
                        MEER INFO
                    </button>
