...
export default function ProfielGaming({
    ...
    registerMinecraft = (() => ({})) as unknown as UseFormRegister<{ minecraft_username?: string | null }>,
        ...
        <Tile title="Social Gaming" icon={<Gamepad2 className="h-5 w-5" />} className="h-fit">
            <div className="rounded-2xl bg-slate-50 dark:bg-black/20 p-5 border border-slate-200 dark:border-white/10 shadow-sm relative group">
                <div className="flex items-center justify-between gap-2 mb-3">
                    <p className="text-base font-bold text-[var(--color-purple-400)] text-left">
                        Minecraft username
                    </p>
                    {!isEditingMinecraft && (
                        <button
                        ...
                        <form onSubmit={handleSubmitMinecraft(onSaveMinecraft)} className="flex flex-col w-full gap-2" autoComplete="off">
                            <div className="flex w-full items-center gap-2">
                                <input
                                    ...
                                    type="text"
                                    className={`flex-1 bg-white dark:bg-black/40 border ${minecraftErrors.minecraft_username ? 'border-red-500 ring-1 ring-red-500' : 'border-slate-300 dark:border-white/20'} rounded-lg px-3 py-1.5 text-base font-medium focus:ring-2 focus:ring-[var(--color-purple-500)] focus:border-transparent outline-none`}
                                ...
                                <button
                                    type="submit"
                                    disabled={isPending}
                                    className="p-1.5 bg-[var(--color-purple-500)] text-white rounded-lg hover:bg-[var(--color-purple-600)] transition-colors cursor-pointer"
                                >
