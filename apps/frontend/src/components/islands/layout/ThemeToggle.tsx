...
export function ThemeToggle() {
    ...
    if (!mounted) {
        return (
            <button
                type="button"
                className="inline-flex items-center justify-center rounded-full bg-(--bg-card)/80 p-2 h-9 w-9 text-transparent shadow-sm"
                aria-hidden="true"
            >
    ...
    return (
        <button
            ...
            className="inline-flex items-center justify-center rounded-full bg-(--bg-card)/80 p-2 h-9 w-9 text-(--text-main) shadow-sm transition-transform duration-200 hover:scale-110 active:scale-95 hover:bg-purple-100 dark:hover:bg-white/10"
        >
            {isDark ? (
