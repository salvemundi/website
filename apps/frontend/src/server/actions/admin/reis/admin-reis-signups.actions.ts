...
import { fetchAllTripSignupsDb, fetchTripSignupByIdDb, fetchTripSignupActivitiesDb, updateTripSignupDb, deleteTripSignupDb, fetchSelectedSignupActivitiesDb } from '@/server/internal/reis/reis-signup-db.utils';
        ...
        if (status === 'confirmed' && oldStatus !== 'confirmed' && signup.email) {
            ...
            if (mailUrl && token) {
                ...
                try {
                    ...
                    safeConsoleError(`[trip-signups.actions.ts][updateSignupStatus] Failed to fetch trip ${signup.trip_id}:`, error);
                }
