...
async function uploadToDirectus(file: File): Promise<string | null> {
    ...
    try {
        ...
        safeConsoleError('[admin-webshop-products.actions.ts][uploadToDirectus] Upload failed:', error);
        return null;
    }
}
...
export async function uploadWebshopMedia(formData: FormData) {
    ...
    const assetId = await uploadToDirectus(file);
    if (!assetId) {
        return { success: false, error: 'Uploaden mislukt.' };
    }

    return { success: true, assetId, assetType: file.type };
}
