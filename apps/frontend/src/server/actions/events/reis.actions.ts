...
import { fetchUserProfileByEmailDb, fetchUserCommitteesDb, type Committee } from '@/server/internal/user-db.utils';
...
export async function createTripSignup(data: ReisSignupForm, tripId: number): Promise<{ success: boolean; message?: string }> {
    ...
    // Destructure without assigning to unused variable email
    const session = await getEnrichedSession();
