...
async function uploadActivityImage(imageFile: File | null): Promise<{ success: true; imageId: string | null } | { success: false; error: string }> {
    ...
    return { success: true, imageId: uploadResult.id };
}

export async function verifyActivityBOLA(eventId: number) {
    const { user } = await enforceFeatureAccess('activiteiten');

    if (canAccess(user.committees, 'commissies')) return user;
    ...
    if (!userCommitteeIds.includes(Number(activityCommitteeId))) {
        throw new Error("Helaas, je mag alleen activiteiten van je eigen commissie beheren.");
    }
...
export async function createActivityAction(prevState: unknown, formData: FormData): Promise<CreateActivityResult> {
    const { isLeader } = await enforceFeatureAccess('activiteiten');
    if (!isLeader) {
        throw new Error("Helaas, alleen commissieleiders en het beheer mogen activiteiten aanpassen.");
    }

    const imageFile = formData.get('imageFile') as File | null;
    const uploadResult = await uploadActivityImage(imageFile);
    if (!uploadResult.success) {
        return { success: false, error: uploadResult.error };
    }
...
export async function updateActivityAction(eventId: number, prevState: unknown, formData: FormData) {
    ...
    try {
        ...
        if (!existing) return { error: "Activiteit niet gevonden", success: false };
