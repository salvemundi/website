...
import { getReisSiteSettings, getUpcomingTrips, getTripSignupsInternal } from './trip-queries.actions';
...
export async function createTripSignup(data: ReisSignupForm, tripId: number): Promise<{ success: boolean; message?: string }> {
    ...
    try {
        ...
        fetch(`${getMailUrl()}/api/mail/send`, {
            ...
            safeConsoleError(`[trip-mutations.actions.ts][createTripSignup] Failed to send trip signup email:`, error);
        });

        return { success: true };
    } catch (error: unknown) {
        safeConsoleError(`[trip-mutations.actions.ts][createTripSignup] Failed to create trip signup for trip ${tripId}:`, error);
...
export async function revalidateReisAction() {
    try {
        ...
        safeConsoleError(`[trip-mutations.actions.ts][revalidateReisAction] Failed to revalidate reis:`, error);
