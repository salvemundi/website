...
export async function getCommittees(): Promise<Committee[]> {
    ...
    try {
        ...
        const committeesJson = await committeesRes.ok ? await committeesRes.json() : { data: [] };
        const committeesData = committeesJson?.data ?? [];
        ...
        const committeesWithMembers = committeesData.map((committee: any) => ({
            ...committee,
            members: allMembers.filter((m: any) => m.committee_id === committee.id)
        }));
