...
import { vacancySubmissionSchema, type VacancySubmissionForm } from '@salvemundi/validations';
...
export async function submitVacancy(data: VacancySubmissionForm) {
    ...
    const parsed = vacancySubmissionSchema.safeParse(data);
    if (!parsed.success) {
