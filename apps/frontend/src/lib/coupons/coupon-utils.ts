...
export function getComputedCouponStatus(coupon: Coupon): {
    ...
    // 1. Check if expired (must be strictly after valid_until day)
    if (coupon.valid_until) {
        const expiryDate = endOfDay(new Date(coupon.valid_until));
        if (expiryDate < now) {
