    ...
    return (
        <div className="w-full text-center">
            ...
            {title && (
                href ? (
                    <div>
                        <Link href={href} className="text-2xl sm:text-3xl font-extrabold text-white hover:text-purple-100 transition drop-shadow-md block mb-4">
                            {title}
                        </Link>
                    </div>
                ) : (
                    <div>
                        <div className="text-2xl sm:text-3xl font-extrabold text-white drop-shadow-md mb-4">{title}</div>
                    </div>
                )
            )}
...
const SlotUnit = ({ value, label }: { value: string | number; label: string }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-1 sm:gap-2">
            ...
            <div className="relative p-0.5 sm:p-1 rounded-lg sm:rounded-xl bg-gradient-to-b from-theme-purple-light to-theme-purple-dark shadow-xl">
                {/* The Recessed Reel Window */}
                <div className="relative bg-[#111] rounded-md sm:rounded-lg p-0.5 sm:p-2 min-w-[2.5rem] sm:min-w-[4.5rem] h-14 sm:h-24 flex items-center justify-center overflow-hidden border-2 border-black/20">
...
const SlotDigit = ({ digit }: { digit: string }) => {
    return (
        <div className="relative w-5 sm:w-9 h-full overflow-hidden flex justify-center items-center bg-[#1a1a1a] rounded-[2px]">
            ...
            <AnimatePresence mode="popLayout">
                ...
                >
                    <span className="text-xl sm:text-4xl font-black font-mono text-theme-purple-dark dark:text-theme-purple-light"
