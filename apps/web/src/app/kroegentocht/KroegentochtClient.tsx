...
interface KroegentochtClientProps {
    initialEvents: PubCrawlEvent[];
    initialSettings: any;
}

export default function KroegentochtClient({ initialEvents, initialSettings }: KroegentochtClientProps) {
    ...
    const isKroegentochtEnabled = initialSettings?.show ?? true;
    const kroegentochtDisabledMessage = initialSettings?.disabled_message || 'De inschrijvingen voor de kroegentocht zijn momenteel gesloten.';
    ...
    const nextEvent = useMemo(() => {
        ...
        return validEvents.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())[0];
    }, [initialEvents]);

    const checkExistingTickets = async (email: string) => {
        if (!email || !nextEvent) return;
        try {
            // Use Server Action for the count
            const count = await checkExistingTicketsAction(email, nextEvent.id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const signupResult = await createPubCrawlSignupAction({
                name: primaryName,
                email: email,
            ...
            // 2. Initiate Payment via Finance Action
            const totalPrice = (finalAmount * 1); // 1 euro per ticket
            ...
            const paymentResult = await createPaymentAction(paymentPayload);

            if (!paymentResult.success) {
                // If payment creation failed, we should probably inform user but the signup exists in 'open' state.
                throw new Error(paymentResult.error || 'Fout bij net aanmaken van de betaling.');
            }
            ...
            let friendlyMessage = 'Er is een onverwachte fout opgetreden.';

            const knownErrors = ['tickets', 'inschrijving', 'emailadres', 'deelnemer', 'letter', 'maximum'];
            if (knownErrors.some(keyword => message.toLowerCase().includes(keyword))) {
                friendlyMessage = message;
            } else {
                friendlyMessage = message; // Show the server error message if present
            }
                                ...
                                {!nextEvent ? (
                                    ...
                                    submitted ? (
                                        ...
                                        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                                            ...
                                            {/* Association */}
                                            <div>
                                                <label htmlFor="association" className="form-label">Vereniging</label>
                                                <select
                                                    id="association"
                                                    name="association"
                                            ...
                                            {/* Amount of Tickets */}
                                            <div>
                                                <label htmlFor="amount_tickets" className="form-label">Aantal tickets</label>
                                                <input
                                                    type="number"
                                                    id="amount_tickets"
                                                    name="amount_tickets"
                                                        ...
                                                        <div key={index} className="bg-white dark:bg-white/5 rounded-xl p-4 space-y-3 border border-slate-200 dark:border-white/10 shadow-sm transition-all hover:shadow-md">
                                                            ...
                                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                                <div className="sm:col-span-2">
                                                                    <label htmlFor={`participant-name-${index}`} className="form-label text-xs uppercase tracking-wider opacity-70">Voornaam + tussenvoegsel</label>
                                                                    <input
                                                                ...
                                                                <div>
                                                                    <label htmlFor={`participant-initial-${index}`} className="form-label text-xs uppercase tracking-wider opacity-70">1e letter achternaam</label>
                                                                    <input
                                                                        id={`participant-initial-${index}`}
                                                                        name={`participant-initial-${index}`}
                                                                        type="text"
