...
import { calculateStickerStats, geocodeAddress, reverseGeocode } from '@/shared/lib/api/utils';
import type { Sticker } from '@/shared/lib/api/types';
import { getStickersAction, createStickerAction, CreateStickerData } from '@/shared/api/sticker-actions';
...
function StickersContent({ initialStickers }: { initialStickers: Sticker[] }) {
    const { user } = useAuth();
    ...
    const { data: stickers = [], isLoading, error } = useQuery<Sticker[]>({
        queryKey: ['stickers'],
        queryFn: () => getStickersAction(),
        initialData: initialStickers,
    });
    ...
    const createMutation = useMutation({
        mutationFn: (data: CreateStickerData) => createStickerAction(data),
        onSuccess: (result) => {
            if (result.success) {
        ...
        onError: (error: any) => {
            ...
            const msg = 'Failed to add sticker location. Please try again.';
            alert('❌ ' + msg);
        },
    });
    ...
    const uploadImage = typeof window !== 'undefined' ? async (file: File): Promise<string | null> => {
        try {
                                ...
                                {mobileFiltersOpen && (
                                    <div className="absolute top-24 left-1/2 -translate-x-1/2 w-[94%] max-w-md bg-white dark:bg-[var(--bg-card)] rounded-2xl shadow-2xl p-4 z-[1100]">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div>
                                                <label htmlFor="mobile-filter-people" className="text-xs text-theme-muted">People</label>
                                                <input
                                                    id="mobile-filter-people"
                                                    name="mobile-filter-people"
                                                    type="text"
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[9999] pt-20">
                    <div className="bg-white dark:bg-[var(--bg-card)] rounded-3xl max-w-4xl w-full max-h-[calc(100vh-6rem)] overflow-y-auto shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                            {/* Address Search */}
                            <div className="mb-6">
                                <label htmlFor="search-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Search Address
                                </label>
                                <div className="flex gap-2">
                                    <input
                            ...
                            {/* Mini Map */}
                            <div className="mb-6">
                                <label htmlFor="map-location-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Select Location on Map
                                </label>
                                <div id="map-location-select" className="h-[300px] rounded-lg overflow-hidden border border-gray-300 dark:border-gray-600">
                                    <StickerMap
                                ...
                                <div className="space-y-3">
                                    <div className="flex gap-2">
                                        ...
                                        <label className="flex-1 cursor-pointer">
                                            ...
                                            <input
                                                id="sticker-image-capture"
                                                name="sticker-image-capture"
                                                type="file"
                                                accept="image/*"
                                                capture="environment"
...
export default function StickersPageClient({ initialStickers }: { initialStickers: Sticker[] }) {
    return (
        <Suspense fallback={
            ...
            <StickersContent initialStickers={initialStickers} />
        </Suspense>
    );
}
